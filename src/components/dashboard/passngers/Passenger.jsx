...
const Passenger = () => {
  ...
  const handleShowModal = (passengerId) => {
    setIsloading(true);
    axios
      .get(`http://localhost:3000/api/v1/passenger/getPassenger/${passengerId}`)
      .then((response) => {
        setSelectedPassenger(response.data.message); // Set the fetched passenger details
  ...
  const deletePassenger = (passengerId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .put(
            `http://localhost:3000/api/v1/passenger/deletePassenger/${passengerId}`
          )
          .then((response) => {
  ...
  useEffect(() => {
    setIsloading(true);
    axios
      .get("http://localhost:3000/api/v1/passenger/allPassengers")
      .then((response) => {
    ...
    <>
      <div className="card shadow" style={{ marginBottom:"150px" }}>
        <div className="card-header py-3 d-sm-flex align-items-center justify-content-between">
          ...
          <h6 className="m-0 font-weight-bold text-primary">All Passengers</h6>
          <Link
          ...
          {selectedPassenger ? (
            <>
              ...
              <ul>
                ...
                <li>
                  <strong>National ID:</strong> {selectedPassenger.nic}
                </li>
                <li>
                  <strong>Registration Type:</strong>{" "}
                  {selectedPassenger.isTemporary
                    ? "By Operator"
                    : "Self Registration"}
                </li>
                <li>
